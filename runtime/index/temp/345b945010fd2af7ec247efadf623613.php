...
<div class="am-popup common-cropper-popup" id="user-avatar-popup">
	...
	<div class="user-content">
    	<div class="user-content-body">
			<?php foreach($safety_panel_list as $v): ?>
				<section class="am-panel am-panel-default am-radius am-panel-<?php if(empty($data[$v['type']]) and empty($v['msg'])): ?>danger<?php else: ?>default<?php endif; ?>">
					<header class="am-panel-hd">
						<h3 class="am-panel-title"><?php echo htmlentities($v['title']); ?></h3>
					</header>
					<div class="am-panel-bd o-h">
						<div class="am-fl panel-left">
							<?php if(!empty($v['msg'])): ?>
								<span><?php echo htmlentities($v['msg']); ?></span>
							<?php endif; if(empty($data[$v['type']]) and !empty($v['tips'])): ?>
								<?php echo htmlentities($v['no_msg']); else: if(!empty($data[$v['type']])): ?>
									<?php echo str_replace('#accounts#', $data[$v['type']], $v['ok_msg']); ?>
								<?php endif; ?>
							<?php endif; if(!empty($v['tips'])): ?>
								<p class="am-text-xs am-margin-bottom-xs"><?php echo htmlentities($v['tips']); ?></p>
							<?php endif; ?>
						</div>
						<?php if(empty($data[$v['type']]) and empty($v['msg'])): ?>
							<a href="<?php echo htmlentities($v['url']); ?>" class="am-btn am-btn-danger am-radius am-btn-xs am-fr">绑定</a>
						<?php else: ?>
							<a href="<?php echo htmlentities($v['url']); ?>" class="am-btn am-btn-secondary am-radius am-btn-xs am-fr">修改</a>
						<?php endif; ?>
					</div>
				</section>
			<?php endforeach; ?>
