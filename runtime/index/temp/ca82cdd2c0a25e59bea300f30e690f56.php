...
<div class="am-popup common-cropper-popup" id="user-avatar-popup">
	...
	<div class="user-content">
		<div class="user-content-body">
			<!-- form start -->
			<form class="am-form form-validation" action="<?php echo MyUrl('index/safety/accountsupdate'); ?>" method="POST" request-type="ajax-url" request-value="<?php echo MyUrl('index/safety/index'); ?>">
				<legend>
					<span class="legend-title">新手机号码校验</span>
					<a href="<?php echo MyUrl('index/safety/index'); ?>" class="am-fr am-margin-top-sm am-icon-mail-reply"> 返回</a>
				</legend>
				<div class="am-form-group">
					<label>手机号码</label>
					<input type="text" name="accounts" class="am-radius" id="accounts" placeholder="手机号码" pattern="<?php echo MyConst('common_regex_mobile'); ?>" data-validation-message="手机号码格式错误" required />
				</div>
				<label>验证码</label>
				<div class="am-input-group am-input-group-sm am-form-group">
					...
					<span class="am-input-group-btn">
						<button class="am-btn am-btn-default am-radius btn-loading-example verify-submit-new" type="button" data-am-loading="{spinner:'circle-o-notch', loadingText:'发送中...'}" data-url="<?php echo MyUrl('index/safety/verifysend'); ?>" data-verify="<?php echo MyC('common_img_verify_state'); ?>" data-text="获取验证码" data-send-text="还有 {time} 秒" data-time="<?php echo MyC('common_verify_interval_time', 30, true); ?>">获取验证码</button>
					</span>
				</div>
				...
				<div class="am-modal-dialog am-radius">
					...
					<div class="am-modal-bd">
						<div class="base">
							...
							<div class="am-fl">
								<img src="<?php echo MyUrl('index/safety/verifyentry'); ?>" class="am-radius c-p" id="verify-img" onClick="this.src=this.src+((<?php echo htmlentities($url_model); ?> == 0) ? '&' : '?')+Math.random();" />
								<a href="javascript:;" class="verify-tips" onClick="document.getElementById('verify-img').src='<?php echo MyUrl('index/safety/verifyentry'); if($url_model == 0): ?>&<?php else: ?>?<?php endif; ?>'+Math.random();">看不清换一张</a>
							</div>
						</div>
						<button type="button" class="am-btn am-btn-secondary am-radius am-btn-sm block m-t-20 btn-loading-example verify-submit-win-new" data-am-loading="{spinner:'circle-o-notch', loadingText:'发送中...'}" data-win="1">确认</button>
					</div>
				</div>
			</div>
			<!-- verify win end -->
