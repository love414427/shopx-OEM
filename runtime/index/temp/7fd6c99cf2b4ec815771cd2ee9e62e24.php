...
<div class="am-popup common-cropper-popup" id="user-avatar-popup">
    ...
    <!-- 是否开启批量支付 -->
    <?php if(MyC('home_is_enable_order_bulk_pay') == 1): ?>
        <button type="button" class="am-btn am-btn-secondary am-radius am-btn-xs am-icon-paypal batch-pay-submit" data-url="<?php echo MyUrl('index/order/pay'); ?>"> 批量支付</button>
    <?php endif; ?>
    <!-- 父级内容 -->
    ...
    <table class="am-table am-table-striped am-table-hover am-table-bordered am-text-nowrap form-table-data-list">
        ...
        <tbody>
            <?php if(!empty($data_list) and !empty($form_table['base']['key_field'])): ?>
                <!-- 处理数据数量小于默认数量 -->
                <?php $__FOR_START_1283513397__=0;$__FOR_END_1283513397__=count($data_list);for($i=$__FOR_START_1283513397__;$i < $__FOR_END_1283513397__;$i+=1){ ?>
                    <!-- 1. tr 主键id名称 -->
                    <!-- 2. 是否黄色选中class -->
                    <tr
