    ...
    <div class="user-content">
        <div class="user-content-body">
			<legend>
				...
				<a href="<?php echo MyUrl('index/personal/saveinfo'); ?>" class="am-fr"> 编辑</a>
			</legend>
			<dl class="dl-content">
				<?php foreach($personal_show_list as $k=>$v): if(isset($user[$k]) or (isset($v['is_ext']) and $v['is_ext'] == 1)): ?>
						<dt><?php echo htmlentities($v['name']); ?></dt>
						<dd>
                            <?php if(isset($v['is_ext']) and $v['is_ext'] == 1): if(isset($v['value'])): ?>
                                    <?php echo htmlentities($v['value']); ?>
                                <?php endif; else: if(empty($user[$k])): ?>
    								<span class="items-value-empty">未填写</span>
    							<?php elseif($k == 'avatar'): ?>
                                    <img src="<?php echo htmlentities($user[$k]); ?>" width="50" height="50" />
                                <?php else: ?>
    								<?php echo htmlentities($user[$k]); ?>
    							<?php endif; ?>
                            <?php endif; if(!empty($v['tips'])): if(!empty($v['path'])): ?>
                                    <span class="span-edit"><?php echo str_replace('{path}', MyUrl($v['path']), $v['tips']); ?></span>
                                <?php else: ?>
                                    <span class="span-edit"><?php echo $v['tips']; ?></span>
                                <?php endif; ?>
							<?php endif; ?>
						</dd>
					<?php endif; ?>
