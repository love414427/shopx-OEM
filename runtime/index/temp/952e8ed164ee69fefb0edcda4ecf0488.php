...
<div class="am-container plugins-payecoquick">
    <div class="am-u-md-8">
        <!-- form start -->
        <form class="am-form form-validation view-save" action="<?php echo PluginsHomeUrl('payecoquick', 'index', 'pay'); ?>" method="POST" request-type="ajax-fun" request-value="FormBackHandle">
            <div class="am-form-group business-form-block">
                <label>真实姓名<span class="am-form-group-label-tips-must">必填</span></label>
                <input type="text" name="name" placeholder="姓名" data-validation-message="请输入姓名" class="am-radius" value="" required />
            </div>
            <div class="am-form-group business-form-block">
                <label>身份证号码<span class="am-form-group-label-tips-must">必填</span></label>
                <input type="text" name="idcard" placeholder="身份证号码" data-validation-message="请输入身份证号码" class="am-radius" value="" required />
            </div>
            <div class="am-form-group business-form-block">
                <label>银行卡号<span class="am-form-group-label-tips-must">必填</span><span class="am-form-group-label-tips">当前身份证开户的银行卡</span></label>
                <input type="text" name="bank_card" placeholder="银行卡号" data-validation-message="请输入银行卡号" class="am-radius" value="" required />
            </div>
            <div class="am-form-group business-form-block">
                <label>手机号码<span class="am-form-group-label-tips-must">必填</span><span class="am-form-group-label-tips">银行预留的手机号码</span></label>
                <input type="text" name="mobile" placeholder="手机号码" data-validation-message="请输入手机号码" class="am-radius" value="" required />
            </div>
            <div class="am-form-group business-form-block">
                <label>验证码<span class="am-form-group-label-tips-must">必填</span></label>
                <div class="am-input-group am-input-group-sm">
                    <input type="number" name="sms_code" class="am-radius am-form-field" placeholder="验证码" min="0" data-validation-message="请输入验证码" autocomplete="off" value="" required />
                    <span class="am-input-group-btn">
                        <button class="am-btn am-btn-default am-radius btn-loading-example verify-submit" type="button" data-am-loading="{spinner:'circle-o-notch', loadingText:'发送中...'}" data-text="获取验证码" data-send-text="还有 {time} 秒" data-time="60" data-url="<?php echo PluginsHomeUrl('payecoquick', 'index', 'verify'); ?>" enctype="multipart/form-data">获取验证码</button>
                    </span>
                </div>
            </div>
            <div class="am-form-group business-form-block pwd-container am-hide">
                <label>银行卡安全码<span class="am-form-group-label-tips-must">必填</span><span class="am-form-group-label-tips">有效期+CVN，如：0314231表示有效期=201403、CVN=231</span></label>
            ...
            <div class="am-form-group business-form-block am-form-group-refreshing am-margin-top-lg am-padding-left-0">
                ...
                <input type="hidden" name="order_id" value="<?php echo htmlentities($data['order_id']); ?>" />
                <input type="hidden" name="order_no" value="<?php echo htmlentities($data['order_no']); ?>" />
                <input type="hidden" name="subject" value="<?php echo htmlentities($data['name']); ?>" />
                <input type="hidden" name="total_price" value="<?php echo htmlentities($data['total_price']); ?>" />
                <input type="hidden" name="notify_url" value="<?php echo htmlentities($data['notify_url']); ?>" />
                <input type="hidden" name="call_back_url" value="<?php echo htmlentities($data['call_back_url']); ?>" />
                <input type="hidden" name="redirect_url" value="<?php echo htmlentities($data['redirect_url']); ?>" />
                <input type="hidden" name="site_name" value="<?php echo htmlentities($data['site_name']); ?>" />
                <input type="hidden" name="check_url" value="<?php echo htmlentities($data['check_url']); ?>" />
                <input type="hidden" name="user_info" value='<?php echo base64_encode(json_encode($data['user'])); ?>' />
    ...
    <div class="am-u-md-4 am-padding-top-sm">
        <div class="am-margin-top-sm">
            <p>支付金额：<strong class="am-text-danger am-text-sm"><?php echo htmlentities($currency_symbol); ?><?php echo htmlentities($data['total_price']); ?></strong> 元</p>
        </div>
        <hr data-am-widget="divider" style="" class="am-divider am-divider-default" />
        <?php if(!empty($plugins_config['right_images'])): ?>
            <img src="<?php echo htmlentities($plugins_config['right_images']); ?>" />
        <?php endif; if(!empty($plugins_config['right_desc'])): ?>
            <div class="am-alert am-alert-warning">
                <p><?php echo implode('<br />', $plugins_config['right_desc']); ?></p>
            </div>
        <?php endif; ?>
    </div>
</div>
